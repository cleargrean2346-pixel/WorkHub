...
export default async function NewPostPage() {
  ...
  const { data: membership } = await supabase.from('organization_members')
    .select('organization_id').eq('user_id', user.id).eq('status', 'approved').limit(1).maybeSingle();
  if (!membership) {
    return <main className="onboarding"><section className="onboarding-card"><p className="eyebrow"><span /> NEW POST</p><h1>승인된 구성원만 글을 작성할 수 있습니다.</h1><p>관리자의 승인을 받은 뒤 다시 시도해 주세요.</p><Link className="primary" href="/workspace">내 공간</Link><Link className="back-link" href="/posts">게시글 목록</Link></section></main>;
  }
  ...
  return <main className="workspace-page">
    ...
    <section className={`workspace-content ${styles.editor}`}>
      <div className="workspace-intro"><p className="eyebrow"><span /> KNOWLEDGE</p><h1>새 게시글 작성</h1><p>Markdown 서식과 이미지 업로드를 이용해 읽기 쉬운 글을 만드세요.</p></div>
      <form action={createPost} className={styles.form}>
        <section className={`workspace-panel ${styles.mainPanel}`}>
          <div className={styles.sectionTitle}><div><h2>내용</h2><p>본문은 Markdown으로 저장되어 나중에 수정할 수 있습니다.</p></div><span>필수 항목</span></div>
          ...
          <label htmlFor="body">본문</label><PostEditor organizationId={membership.organization_id} />
        </section>
