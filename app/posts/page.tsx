...
export default async function PostsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const allPosts = (rows ?? []) as Post[];
  const normalizedQuery = q.trim().toLowerCase();
  const posts = normalizedQuery ? allPosts.filter((post) => `${post.title} ${post.body}`.toLowerCase().includes(normalizedQuery)) : allPosts;
  return <main className="workspace-page"><header className="workspace-header"><Link className="brand" href="/"><span className="brand-mark">W</span><span>workhub</span></Link><div><strong>Knowledge</strong><small>Organization posts</small></div><Link href="/workspace" className="back-link">Workspace</Link></header><section className="workspace-content"><div className="workspace-intro"><p className="eyebrow"><span /> KNOWLEDGE</p><h1>Share what your team knows</h1><p>Write updates, notes, and useful guides for your workspace.</p><Link className="primary" href="/posts/new">Write post</Link></div><section className="workspace-panel"><form className="task-form" action="/posts"><label htmlFor="search">Search posts</label><input id="search" name="q" defaultValue={q} placeholder="Search title or content" /><button className="primary">Search</button></form><div className="workspace-panel-title"><h2>{normalizedQuery ? 'Search results' : 'All posts'}</h2><span>{posts.length}</span></div>{posts.length ? <div className="live-tasks">{posts.map((post) => <Link className="live-task" href={`/posts/${post.id}`} key={post.id}><div><b>{post.title}</b><small>{post.status} · {new Date(post.created_at).toLocaleDateString('ko-KR')} · {post.body.slice(0, 120) || 'No content yet.'}</small></div></Link>)}</div> : <div className="empty-state">No matching posts yet.</div>}</section></section></main>;
}
