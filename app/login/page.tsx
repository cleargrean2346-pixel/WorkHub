...
export default function LoginPage() {
  ...
  const submit = async (event: React.FormEvent) => { event.preventDefault(); setLoading(true); setNotice(''); const { error } = await createClient().auth.signInWithOtp({ email, options: { emailRedirectTo: `${location.origin}/auth/callback` } }); const errorMessage = typeof error?.message === 'string' && error.message.trim() ? error.message : typeof error?.code === 'string' ? error.code : 'SMTP 설정 또는 발신 이메일을 확인해 주세요.'; setNotice(error ? `로그인 링크 전송 오류: ${errorMessage}` : '로그인 링크를 이메일로 보냈습니다. 받은편지함과 스팸함을 확인하세요.'); setLoading(false); };
