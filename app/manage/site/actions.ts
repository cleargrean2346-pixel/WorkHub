...
export async function updateSiteSettings(formData: FormData) {
  ...
  const { error } = await supabase.from('site_settings').update({ site_title: siteTitle, site_description: siteDescription, maintenance_enabled: maintenanceEnabled, updated_by: user.id, updated_at: new Date().toISOString() }).eq('id', true);
