...
export default async function MembersPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return <main className="workspace-page">
    ...
    <section className={`workspace-content ${styles.membersPage}`}>
      ...
      <section className="workspace-panel"><div className="workspace-panel-title"><div><h2>구성원 목록</h2><p className={styles.helper}>{visibleMembers.length}명 표시 · 최고관리자는 강등 또는 변경할 수 없습니다.</p></div><span>{selectedRole === 'all' ? '전체' : roleLabels[selectedRole]}</span></div>{visibleMembers.length ? <div className={styles.memberList}>{visibleMembers.map((member) => { const profile = Array.isArray(member.profiles) ? member.profiles[0] : member.profiles; const locked = member.role === 'system_admin'; const name = profile?.full_name || profile?.email || '이름 없는 구성원'; return <form className={styles.memberRow} action={changeMemberRole} key={member.user_id}><input type="hidden" name="organizationId" value={current.organization_id}/><input type="hidden" name="userId" value={member.user_id}/><span className={styles.avatar}>{name.slice(0, 1).toUpperCase()}</span><div className={styles.person}><b>{name}</b><small>{profile?.email || '이메일 없음'} · {member.status === 'approved' ? '승인됨' : '승인 대기'}</small></div><span className={`${styles.badge} ${locked ? styles.system : ''}`}>{roleLabels[member.role] || member.role}</span><select name="role" defaultValue={member.role} disabled={member.status !== 'approved' || locked} aria-label={`${name}의 역할`}><option value="member">구성원</option><option value="team_leader">팀 리더</option><option value="manager">매니저</option><option value="organization_admin">조직 관리자</option></select><button className="secondary" disabled={member.status !== 'approved' || locked}>{locked ? '보호됨' : '권한 저장'}</button></form>; })}</div> : <div className="empty-state">조건에 맞는 구성원이 없습니다.</div>}</section>
    </section>
  </main>;
}
