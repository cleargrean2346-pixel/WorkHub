...
export default async function MembersPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [{ data: memberRows }, { count: pendingCount }] = await Promise.all([
    supabase.from('organization_members').select('user_id,role,status').eq('organization_id', current.organization_id).eq('status', 'approved').order('joined_at'),
    supabase.from('organization_members').select('*', { count: 'exact', head: true }).eq('organization_id', current.organization_id).eq('status', 'pending'),
  ]);
  ...
  return <main className="workspace-page">
    ...
    <section className={`workspace-content ${styles.membersPage}`}>
      ...
      <section className={`workspace-panel ${styles.filters}`}>
        ...
        <nav className={styles.roleTabs} aria-label="권한별 구성원"><Link href="/manage/members" className={selectedRole === 'all' ? styles.active : ''}>전체 <b>{countByRole.all}</b></Link>{roles.slice(1).map(([role, label]) => <Link href={`/manage/members?role=${role}${query ? `&q=${encodeURIComponent(params.q ?? '')}` : ''}`} className={selectedRole === role ? styles.active : ''} key={role}>{label} <b>{countByRole[role]}</b></Link>)}<Link href="/manage/members/pending">승인 대기 <b>{pendingCount ?? 0}</b></Link></nav>
      </section>
